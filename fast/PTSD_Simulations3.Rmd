...
### Distribution of Recovery Trajectories
...
```{r}
a <- a %>% 
  arrange(Run, PTEV, PTES, PTES, NumAttributes, RuminationFrequency, W, Gamma)

categorized <- a %>% 
  group_by(Run, PTEV, PTES, PTES, NumAttributes,
           RuminationFrequency, W, Gamma) %>%
  summarize(Trajectory = classify(Traumatic))

csummary <- categorized %>%
  #mutate(Class=as_factor(class)) %>%
  group_by(PTEV, Gamma, Trajectory) %>%
  summarize(Num=n() / 24)

ggplot(data=csummary, aes(x="", y=Num, fill=Trajectory)) +
...
Finally, the last factor we examined is the effect of the similarity __PTES__ between the PTE and the contextual events (_Q_). 
...
ggplot(data=filter(a, PTEV != 1), aes(x=Day, y=Traumatic, col=PTES)) +
  ...
  facet_grid(PTEV ~ Gamma, labeller = label_both) +
  theme_pander() +
  ggtitle("Effect of PTE similarity on Intrusion Probability")
