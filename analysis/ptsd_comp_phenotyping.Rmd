...
if (CREATE_MLE) {
  ...
  write_csv(mle_prediction, "mle_predictions5.csv")
} else {
  mle_predictions <- read_csv("mle_predictions5.csv")
}
```
...
ggplot(newbackpred, aes(x=SPredicted, y=SObserved, fill=Percent)) +
  ...
  #geom_smooth(method="lm", col="red") +
  facet_wrap(~Parameters, scales="free", ncol=2) +
  geom_text(aes(label = percent(Percent, .1)), col="white") +
...
```{r}
accuracies$Chance = c(1/4, 1/3, 1/3, 1/2, 1/3)

ggplot(accuracies, aes(x=Parameters, y=Accuracy, fill=Parameters)) +
  scale_fill_aaas() +
  facet_wrap(~ Parameters, ncol=5, drop=TRUE, scales = "free_x") +
