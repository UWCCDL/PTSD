...
ggplot(wroc, aes(x=x, y=y, col=Trajectories)) +
  ...
  scale_x_reverse() +
  ggtitle("Prediction Accuracy for Trajectories") +
...
```{r}
aucs <- foreach(param = unique(newbackpred$Parameters), .combine='cbind') %:% {
  foreach(mday = unique(newbackpred$MaxDay)) %dopar% {
    ...
    mroc <- mmulticlass.roc(sub$SObserved, sub$SPredicted)
