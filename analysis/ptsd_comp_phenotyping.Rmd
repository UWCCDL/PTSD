...
newbackpred <- wmle_predictions %>%
  ...
  ungroup() %>%
  group_by(Parameters, SObserved, SPredicted) %>%
  summarise(Count = length(Run)) %>%
  ungroup() %>%
  group_by(Parameters, SObserved) %>%
...
ggplot(accuracies, aes(x=Parameters, y=Accuracy, fill=Parameters)) +
  facet_wrap(~ Parameters, ncol=4, drop=TRUE, scales = "free_x") +
