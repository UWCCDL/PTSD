...
a <- a %>%
  ...
  mutate(A = as_factor(NumAttributes)) %>%
  mutate(C = as_factor(PTES)) %>%
  mutate(R = as_factor(RuminationFrequency))
```
...
a$Trajector <- factor(a$Trajectory,
                      levels = c("Resilient", "Delayed", "Recovery", "Chronic"))
```

### Finding a baseline to compare the model to existing literature

Since we have no idea how typical the model parameters are of the existing patients and their cases, we need to provide some sort of baselines. To do so, we will use the proportion of trajectories that weregiven by Galatzer-Levy and Bonanno in their meta-analysis. They are:
...
```{r}
T_observed <- c(0.65, 0.23, 0.12, 0.1)
...
ggplot(test, aes(x=Chi, y=r, col=RMSE)) + 
  ...
  ggtitle("Relationship between Error Measures in Trjectories") +
  theme_pander()
```
...
Distributions of Trajectories Across Factors 
