...
#  geom_point(alpha=0.1) +
  ...
  scale_color_viridis(expression(gamma), discrete=T, 
                      option="plasma", begin = 0, end=0.9) +
  geom_vline(data=props, aes(xintercept=-0.15)) +
  scale_fill_viridis(expression(gamma), option = "plasma", 
...
```{r}
mylogit <- glm(Recovery ~ (W + gamma), family="binomial", 
               data=filter(Wg_patterns, W<12))
summary(mylogit) %>%
  xtable() %>%
  kable() %>%
  kable_styling(bootstrap_options = c("hover", "striped"))
...
```{r}
mylogit <- glm(Delayed ~ (W * gamma), family=binomial("logit"), data=Wg_patterns )
summary(mylogit) %>%
  xtable() %>%
  kable() %>%
  kable_styling(bootstrap_options = c("hover", "striped"))
