...
## Add a little bump to resilient

resilient <- resilient + recovery * 0.075
delayed <- delayed + recovery * 0.075
...
ggplot(ideal, 
  ...
  ylab("Severity of Intrusive Re-experiencing") +
...
U <- dgamma(t, shape=6, rate=1/100)

dists <- data.frame(Time=t, Events = E, Rumination = U) %>%
  pivot_longer(names_to = "Type", 
               values_to = "Probability",
               cols=c("Events", "Rumination"))
dists$Type[dists$Type == "Rumination"] <- "Unproductive Processing"
...
ggplot(baseline_a, 
       aes(x=Day, y=CTraumatic, col=Trajectory)) +
  stat_summary(geom="line") +
  ggtitle("Trajectories for Best-Fitting Parameters") +
  scale_color_d3() +
  geom_label(data=base_total, aes(x= c(25, 10, 20, 10), y = MeanC, 
                                 label=percent(Prop, accuracy = 0.1)),
            vjust=-.5, show.legend=F) +
  ylab("Total Number of Memory Intrusions") +
  geom_vline(data=props, aes(xintercept=-0.25)) +
...
# lm.beta(IC_mod) 
#or
# glm(scale(CTraumatic) ~ scale(I) * scale(C), family="poisson", IC_traumatic)
summary(IC_mod) %>%
  xtable %>%
  kable %>%
...
And the standardized betas

```{r}
summary(lm.beta(IC_mod))$coefficients %>% 
  xtable() %>%
  kable %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
summary(lm.beta(mylogit))$coefficients %>%
  xtable() %>%
  kable() %>%
  kable_styling(bootstrap_options = c("hover", "striped"))
```
...
```{r}
summary(lm.beta(Wg_mod))$coefficients %>% 
  xtable() %>%
  kable %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
Because of the large varince in the proportion of trajectories (due to the massive effects of _W_), none of the interaction terms are significant. Thus, we analyzed all four trajectories with a simpler generalized linear model of the form $Y = \beta_0 + \beta_W W + \beta_{\gamma} \gamma  + \epsilon$, using a logistic link function. 
...
ggplot(R_a, 
  ...
  scale_color_viridis(expression(paste(italic(U), " =")), discrete=T, option="plasma", end=0.8) +
  scale_fill_viridis(expression(paste(italic(U), " =")), option = "plasma", discrete = T, end=0.8) +
  theme_pander() +
  theme(legend.position = "bottom")
```
...
```{r}
summary(lm.beta(R_mod))$coefficients %>% 
  xtable() %>%
  kable %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
ggplot(data=R_test, aes(x="", y=N/100, fill=Trajectory)) +
  ...
  ylab("Rumination") +
  xlab("") +
  ggtitle("Effect of Unproductive Processing on Trajectories") +
  geom_text_repel(aes(label=percent(N/100, .1)),
            position=position_stack(vjust=0.5), size=3, col="white") +
  #geom_text_repel() +
  theme_pander() +
...
hsize <- a %>%
  ...
  dplyr::summarize(H = h_entropy(MemoryEntropy) - H1,
            ...
            PGroup = if_else(t.test(CTraumatic, mean=0)$p.value > 0.05, "Control", "PTSD"),
            Trajectory = )
```
...
clinically_recovered <- function(vals) {
  if (length(unique(vals)) > 1) {
    pval <- t.test(vals, mean=0)$p.value
  } else {
    if (unique(vals)[1] == 0) {
      pval <- 1
    } else {
...
hsize_pgroup <- a %>%
  filter(Day > 49, I != 1) %>%
  group_by(Run, I, C, R, W, gamma) %>%
  summarise(HippocampusDecrease = 100*(mean(MemoryEntropy)/H1 - 1),
            PTraumatic = mean(PTraumatic),
            PGroup = clinically_recovered(CTraumatic),
            CTraumatic = sum(CTraumatic),
            MeanSimilarity = mean(ChunkSimilarity))

hpc_pgroup <- hsize_pgroup %>%
  group_by(PGroup) %>%
  summarise(VolumeChange = mean(HippocampusDecrease),
            SD = sd(HippocampusDecrease)) %>%
  add_column(region = "parahippocampal") 

#hpc_pgroup$Trajectory <- as.character(hpc_group$Trajectory)
...
p1 <- hpc_pgroup %>%
  ...
  ggtitle("Hippocampus Volume Decrease by Experimental Group") +
  theme_brain2(text.family = "sans",
               text.colour="black") +
  theme(strip.background = element_blank(),
        strip.text = element_text(size=10, face="bold"),
...
p2 <- ggplot(hsize_pgroup, 
             aes(x = "",
                 y = HippocampusDecrease)) +
  geom_violin(aes(fill=PGroup), alpha=0.5, 
              col="white", width=1.05, bw=2) +
  scale_fill_npg() +
  stat_summary(geom="point", fun.data="mean_sdl") +
  facet_wrap(~ PGroup, ncol=4) +
...
As expected, the the difference between simulated control groups (Traumatized controls and Traumatized PTSD) is significant to a two-sided, unequal variance _t_-test.
...
```{r}
t.test(HippocampusDecrease ~ PGroup, 
       hsize_pgroup, 
       var.equal=F) %>%
  tidy() %>%
  kable() %>%
...
Finally, the effect in the Control group is significantly different than zero:

```{r}
with(hsize_pgroup, t.test(HippocampusDecrease[PGroup == "Control"], mu=0)) %>%
  tidy() %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
#### By End-point experimental groups

First, let's visualize the data

```{r fig.width=6, fig.height=5}
fconn <- hsize_pgroup %>%
  filter(!is.na(MeanSimilarity),
         C == 0)

fc <- fconn %>%
  group_by(PGroup) %>%
  summarise(FC = mean(MeanSimilarity),
            SD = sd(MeanSimilarity))
...
p1 <- fc_group %>%
  ...
  theme_brain2(text.family = "sans",
               text.colour="black") +
  ggtitle("Functional Connectivity Between\nMedial PFC and Hippocampus by Experimental Group") +
  theme(strip.background = element_blank(),
        strip.text = element_text(size=10, face="bold"),
        legend.position = "right",
        legend.title = element_text(size=10))
...
conn_group %>%
  ...
  dplyr::select(Group, Trajectory, FC, SD) %>%
  xtable() %>%
  kable() %>%
  kable_styling(bootstrap_options=c("striped", "hover"))
```
...
t.test(MeanSimilarity ~ PGroup, fconn,
       pairted=F,
       var.equal=F)
```
