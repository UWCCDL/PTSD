...
intermediate <- a %>%
  ...
  mutate(Condition = paste("(", I, 
                           ", ",  C, 
                           ", ", W, ")", sep="")) %>%
  group_by(Day, I, C, gamma, W, Condition)  %>%
...
```{r}
get_prob_by_day <- function(n, day, Intensity, Context, WM, gammaval) {
    intermediate %>%
    filter(Day == day,
           W == WM,
           I == Intensity,
...
if (CREATE_LOGLIKELIHOODS) {
  ...
  for (n in 0:50) {
    ...
    for (d in 1:20) {
      for (i in c(20, 40, 60)) {
        for (c in c(0, 0.25, 0.5, 0.75)) {
          for (w in c(4, 8, 12)) {
            for (g in c(0.8, 0.9, 0.95)) {
              p <- get_prob_by_day(n, d, i, c, w, g)
              result <- c(n, d, i, c, w, g, p)
              if (is.null(vals)) {
                vals <- result
              } else {
  ...
  vals <- as_tibble(vals)
  names(vals) <- c("N", "Day", "I", "C", "W", "gamma", "Probability")
  
  #Replace probabilities=0 with very small values 
  vals$Probability[vals$Probability == 0] <- 1e-50
  vals$LogLikelihood <- log(vals$Probability)
  write_tsv(vals, "loglikelihoods.tsv")
} else {
...
if (CREATE_MLE) {
              ...
              for (j in unique(a_sub$Run)) {
                ...
                values <- a_sub %>%
                  filter(W == w,  
                         ...
                         Run == j) %>% 
                  dplyr::select(CTraumatic)
                prediction <- as_vector(parameters_mle(values))
                
                observed <- tibble(Parameters = c("I", "C", "W", "gamma"),
                                   Type = "Observed",
                                   Value = c(i, c, w, g),
                                   LogLikelihood = prediction[5],
                                   Run = jj)
                
                predicted  <- tibble(Parameters = c("I", "C", "W", "gamma"),
                                     Type = "Prediction",
                                     Value = prediction[1:4],
                                     LogLikelihood = prediction[5],
                                     Run = jj)
  ...
  write_csv(mle_prediction, "mle_predictions.csv")
} else {
  mle_predictions <- read_csv("mle_predictions.csv")
}
```
...
```{r}
accuracies$Chance = c(1/4, 1/3, 1/3, 1/3)

ggplot(accuracies, aes(x=Parameters, y=Accuracy, col=Parameters)) +
  facet_wrap(~Parameters, ncol=2, drop=TRUE, scales = "free_x") +
  geom_col(col="lightgrey", fill="grey") +
...
wmle_predictions %>%
  ...
  summarise(Predicted = mean(Prediction),
            ...
            PredictedSD = sd(Prediction),
            ObservedSD = sd(Observed))
```
...
ggplot(filter(forward_prediction, I > 1), aes(x=PredictedTrajectory, y=Trajectory)) +
  geom_bin_2d() +
  scale_fill_viridis()
```
