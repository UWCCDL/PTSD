...
ggplot(baseline_a, 
       aes(x=Day, y=PTraumatic, col=Trajectory, fill=Trajectory)) +
  geom_smooth(method="loess", span=0.2) +
  coord_cartesian(ylim=c(0, 0.2)) +
  ggtitle("Baseline Condition") +
  scale_color_jama() +
...
The effects of _I_ and _C_ on the priobabilities of recall during the Chronic period can be understood with a linear model analysis
...
ggplot(data=IC_trajectories, aes(x="", y=N/100, fill=Trajectory)) +
  ...
  facet_wrap(~I*C, labeller = label_both) +
  ...
  ylab("PTE Intensity") +
  xlab("") +
  ggtitle("Effect of PTE Intensity on Trajectories") +
...
```{r}
ggplot(Wg_a, 
       aes(x=Day, y=CTraumatic, col=as.factor(gamma), fill=as.factor(gamma))) +
...
ggplot(data=Wg_test, aes(x="", y=N/100, fill=Trajectory)) +
  ...
  facet_wrap(~ W*gamma, labeller = label_both) +
  coord_polar("y", start=0) +
  #scale_fill_d3() +
  scale_y_continuous(labels = scales::percent_format(accuracy = 2L)) +
  scale_fill_jama() +
...
```{r}
mylogit <- glm(Resilient ~ (W * gamma), family="binomial", data=filter(Wg_patterns, W!=12))
summary(mylogit) %>%
  xtable() %>%
  kable() %>%
  kable_styling(bootstrap_options = c("hover", "striped"))
...
```{r}
mylogit <- glm(Chronic ~ (W * gamma), family="binomial", data=filter(Wg_patterns, W!=13))
summary(mylogit) %>%
  xtable() %>%
  kable() %>%
  kable_styling(bootstrap_options = c("hover", "striped"))
...
```{r}
mylogit <- glm(Recovery ~ (W * gamma), family="binomial", data=Wg_patterns)
summary(mylogit) %>%
  xtable() %>%
  kable() %>%
  kable_styling(bootstrap_options = c("hover", "striped"))
...
```{r}
mylogit <- glm(Delayed ~ (W * gamma), family="binomial", data=filter(Wg_patterns, W<=20))
summary(mylogit) %>%
  xtable() %>%
  kable() %>%
  kable_styling(bootstrap_options = c("hover", "striped"))
...
Finally, we will examine how the model captures some known effects of the neurobiology of PTSD.
...
ggplot(filter(hsize, I != 1),
       aes(x=NumTraumatic, y=HippocampusDecrease, col = External)) +
...
```{r}
ggplot(filter(hsize, I != 1),
       aes(x=MeanTraumatic, y=HippocampusDecrease, col = as.factor(I))) +
