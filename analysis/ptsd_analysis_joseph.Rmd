...
```{r}
a$SSRI <- as_factor(a$SSRI)
a$gamma <- as.character(a$gamma)
a$gamma <- as_factor(a$gamma)
a <- a %>%
  filter(gamma %in% c("0.9", "0.95"))
#a$gamma <- as_factor(a$gamma)
ggplot(a, 
