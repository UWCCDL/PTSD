...
#         PTES==0) %>%
  group_by(Trajectory, I, gamma, W,
           RuminationFrequency, PTES) %>%
...
props %>%
  pivot_wider(id_cols=c(I, gamma, W,
                        RuminationFrequency, PTES), 
...
mychi <- function(l) {
  chisq.test(l, #c(a, b, c, d), 
             p=observed, 
             rescale.p=T)$statistic
}

ltest <- test %>%
  pivot_longer(cols=c("Resilient", "Chronic",
                      "Delayed", "Recovery"),
               names_to = "Trajectory",
               values_to = "N")

ltest <- ltest %>%
  group_by(I,gamma, W, RuminationFrequency, PTES) %>%
  summarize(Chi=mychi(N))
